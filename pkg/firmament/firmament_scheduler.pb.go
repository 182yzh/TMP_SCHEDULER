...
import math "math"
import "math/rand"
import "github.com/kubernetes-sigs/poseidon/pkg/flowscheduler"
...
func NewFirmamentSchedulerClient() FirmamentSchedulerClient {
	glog.Infof("New Firmament Scheduler Client, Create FlowScheduler")
	fc := &firmamentSchedulerClient{}
	fc.fs = flowscheduler.NewFlowScheduler()
	return fc
}

func (c *firmamentSchedulerClient) Schedule() (*SchedulingDeltas, error) {
	//out := new(SchedulingDeltas)
	scheduleResult,taskKilled := c.fs.ScheduleAllJobsToPoseidon()
	//fmt.Println(scheduleResult)
	deltas := new(SchedulingDeltas)
	for tid,rid := range taskKilled{
		td := c.fs.TaskIDToTaskDes(tid)
        rd := c.fs.ResIDToResDes(rid)
       	delta := new(SchedulingDelta)
        delta.TaskId = tid
        delta.ResourceId = rd.Uuid
        delta.Type = SchedulingDelta_PREEMPT
		deltas.Deltas = append(deltas.Deltas,delta)
		glog.Infof("SCHEDULE INFO:KILLED %s running in %s,GPU Need:%d",td.Name,rd.Name,td.ResRequest.Gpu)
		//fmt.Println(time.Now().Format("2006-01-02 15:04:05")," SCHEDULE INFO:KILLED",td.Name,rd.Name,td.ResRequest.Gpu)
    }
	for tid,rid := range scheduleResult{
        delta := new(SchedulingDelta)
        deltas.Deltas = append(deltas.Deltas,delta)
        delta.TaskId = tid
        td := c.fs.TaskIDToTaskDes(tid)
        rd := c.fs.ResIDToResDes(rid)
        delta.ResourceId = rd.Uuid
        delta.Type = SchedulingDelta_PLACE
		glog.Infof("SCHEDULE INFO:PLACE %s schedule to %s ,Gpu need:%d",td.Name,rd.Name,td.ResRequest.Gpu)
        //fmt.Println(time.Now().Format("2006-01-02 15:04:05")," SCHEDULE INFO:PLACE",td.Name,rd.Name,td.ResRequest.Gpu)
    }
...
func (c *firmamentSchedulerClient) TaskCompleted(in *TaskUID) error {
	glog.Infof("SCHEDULE INFO:Task Completed, %d \n",in.GetTaskUid())
	c.fs.HandleTaskCompeletedByID(in.GetTaskUid())
	return  nil
}

func (c *firmamentSchedulerClient) TaskFailed(in *TaskUID) ( error) {
	glog.Infof("SCHEDULE INFO: Task Failed,TaskUID :%v",in.GetTaskUid())
//	c.fs.HandleTaskCompeletedByID(in.GetTaskUid())
	c.fs.HandleTaskFailedByID(in.GetTaskUid())
	return  nil
}

func (c *firmamentSchedulerClient) TaskRemoved(in *TaskUID) (error) {
	glog.Infof("SCHEDULE INFO:Task Removed,TaskUID :%v",in)
	//c.fs.HandleTaskCompeletedByID(in.GetTaskUid())
	c.fs.HandleTaskFailedByID(in.GetTaskUid())
	return  nil
}
...
//=======================
func (c *firmamentSchedulerClient) TaskSubmitted(in *TaskDescription ) ( error) {
	glog.Infof("task submitted to flowscheduler,TaskDescription:%v",in)
//	glog.Infof("task Descriptor:%v",in.TaskDescriptor)
//	glog.Infof("Job Descriptor:%v",in.JobDescriptJor)
	jd := c.TaskDesToFSJobDes(in)
//	fmt.Println(jd)
	jds := make([]*flowscheduler.JobDescriptor,0,0)
	jds = append(jds,jd)
	//fmt.Println(jds)
	if c.fs.ChangeJobDes(jd) == false {
		c.fs.AddJobs(jds)
	}
	return  nil
}

func (c *firmamentSchedulerClient) TaskUpdated( in *TaskDescription) (error) {
	return  nil
}

func (c *firmamentSchedulerClient) NodeAdded(in *ResourceTopologyNodeDescriptor) ( error) {
	rds := c.RTNDtoFSResDes(in)
	//fmt.Println("in fc.Nodeadded!,rtnd is :",in)
	//fmt.Println(rds)
	for _,rd := range rds{
		fmt.Println("in fc.Nodeadded, td is ",rd)
		if rd == nil {
			continue
		}
		c.fs.AddResource(rd)
...
func (c *firmamentSchedulerClient) NodeFailed( in *ResourceUID) ( error) {
	return nil
}

func (c *firmamentSchedulerClient) NodeRemoved( in *ResourceUID) (error) {
	return  nil
}

func (c *firmamentSchedulerClient) NodeUpdated(in *ResourceTopologyNodeDescriptor) ( error) {
	return  nil
}

func (c *firmamentSchedulerClient) AddTaskStats(in *TaskStats) (error) {
	return nil
}

func (c *firmamentSchedulerClient) AddNodeStats(in *ResourceStats) (error) {
	return  nil
}

func (c *firmamentSchedulerClient) Check(ctx context.Context, in *HealthCheckRequest, opts ...grpc.CallOption) ( error) {
	_ = new(HealthCheckResponse)
 
	// c.cc.Invoke(ctx, "/firmament.FirmamentScheduler/Check", in, out, opts...)
	return nil
}

func (c *firmamentSchedulerClient) AddTaskInfo( in *TaskInfo) (error) {
	return nil
}
...
func (c *firmamentSchedulerClient)TaskDesToFSJobDes(in *TaskDescription)*flowscheduler.JobDescriptor{
	//fmt.Println("Task Des To FsJobDes",in)
//	fmt.Println(time.Now().Format("2006-01-02 15:04:05"," SSHEDULE INFO, SUBMIT:",fmt.Sprintf("%s|%d|%d",td.Name,td.Priority,td.ResRequest.Gpu))
	td := new(flowscheduler.TaskDescriptor)
	jd := new(flowscheduler.JobDescriptor)
    td.Tid = in.TaskDescriptor.Uid
	td.Name = in.TaskDescriptor.Name
	td.State = flowscheduler.TASK_UNSCHEDULED
	td.Jd = jd
	td.ResRequest.Gpu = uint64(in.TaskDescriptor.ResourceRequest.GpuCores/2.0)
	td.ResRequest.Cpu = uint64(in.TaskDescriptor.ResourceRequest.CpuCores)
    td.SubmitTime = int64(in.TaskDescriptor.SubmitTime)
	if td.ResRequest.Gpu != 0 {
		td.Priority++
	}
	jd.Name = in.JobDescriptor.Name
	jid := rand.Uint64()
	for c.fs.UsedJobID(jid){
		jid = rand.Uint64()
	}
	jd.Jid = jid
	jd.Uuid = in.JobDescriptor.Uuid
	jd.TaskUnscheduledNum = 1
	jd.SubmitTime = in.TaskDescriptor.SubmitTime
	jd.State = flowscheduler.JOB_CREATED
	jd.Tasks = append(jd.Tasks,td)
	//fmt.Println(jd)
	//fmt.Println(time.Now().Format("2006-01-02 15:04:05")," SSHEDULE INFO, SUBMIT:",fmt.Sprintf("%s|%d|%d",td.Name,td.Priority,td.ResRequest.Gpu))
	glog.Infof("SCHEDULE INFO, SUBMIT: %s|%d|%d",td.Name,td.Priority,td.ResRequest.Gpu)
	return jd
}


func (c *firmamentSchedulerClient) ParseRTND(in *ResourceTopologyNodeDescriptor)[]*flowscheduler.ResDescriptor{
	rd := in.ResourceDesc
	fsrd := c.transFormToFSRD(rd)
	ans := make([]*flowscheduler.ResDescriptor,0,0)
	ans = append(ans,fsrd)
...
func (c *firmamentSchedulerClient)transFormToFSRD(rd *ResourceDescriptor )*flowscheduler.ResDescriptor{
	labels := rd.GetLabels()
//	fmt.Println(labels)
	havelabel := false
	for _,lab := range labels{
		if lab.GetKey() == "schedulerName" && lab.GetValue() != "octopus" {
			return nil
		}else if lab.GetKey() == "schedulerName" && lab.GetValue() == "octopus"{
			havelabel = true
		}
	}
	if havelabel == false {
	...
	ans := new(flowscheduler.ResDescriptor)
	ans.Rtype = flowscheduler.RES_PU
	rid := rand.Uint64()
	for c.fs.UsedResID(rid){
		rid = rand.Uint64()
	}
	ans.Rid =  rid
	ans.Uuid = rd.Uuid
	ans.CurrentRunningTasks = make([]flowscheduler.TaskID,0,0)
    if rd.AvailableResources != nil {
		ans.ResAvailable.Gpu = uint64(rd.AvailableResources.GpuCores/2.0)
		ans.ResAvailable.Cpu = uint64(rd.AvailableResources.CpuCores)
	}
	if rd.ResourceCapacity != nil {
		ans.ResTotal.Gpu = uint64(rd.ResourceCapacity.GpuCores/2.0)
		ans.ResTotal.Cpu = uint64(rd.ResourceCapacity.CpuCores)
	}
	ans.Name = rd.FriendlyName
	//fmt.Println("in transFormToFSRD,fsrd =",ans)
	return  ans

}


func (c *firmamentSchedulerClient) RTNDtoFSResDes(in *ResourceTopologyNodeDescriptor) []*flowscheduler.ResDescriptor{
		//temans := make([]*flowscheduler.ResDescriptor,0,0)
		//var ans []*flowscheduler.ResDescriptor = temans[:]
		ans := c.ParseRTND(in)
		return ans
}
