...
func (pw *PodWatcher) getCPUMemEphemeralRequest(pod *v1.Pod) (int64, int64, int64, int64) {
	cpuReq := int64(0)
	gpuReq := int64(0)
	memReq := int64(0)
	...
	for _, container := range pod.Spec.Containers {
		...
		gpuReqQuantity := request[NVIDIAGPU];
		gpuReq += gpuReqQuantity.ScaledValue(0) * NVIDIAGPUMLTI;
	...
	return cpuReq, gpuReq, memReq, ephemeralReq
}
...
func (pw *PodWatcher) parsePod(pod *v1.Pod) *Pod {
	cpuReq, gpuReq, memReq, ephemeralReq := pw.getCPUMemEphemeralRequest(pod)
...
func (pw *PodWatcher) enqueuePodUpdate(key, oldObj, newObj interface{}) {
	...
	oldCPUReq, oldGPUReq, oldMemReq, oldEphemeralReq := pw.getCPUMemEphemeralRequest(oldPod)
	newCPUReq, newGPUReq, newMemReq, newEphemeralReq := pw.getCPUMemEphemeralRequest(newPod)
	if oldCPUReq != newCPUReq || oldGPUReq != newGPUReq ||
		oldMemReq != newMemReq || oldEphemeralReq != newEphemeralReq ||
			...
			//	FirMux := new(sync.Mutex)
				for _, item := range items {
					...
					case PodDeleted:
						...
						if pod.Identifier == TaskIDToPod[td.GetUid()] {
...
func (pw *PodWatcher) updateTask(pod *Pod, td *firmament.TaskDescriptor) {
	// TODO(ionel): Update LabelSelector! //GPUorCPURequest
	td.ResourceRequest.CpuCores = float32(pod.GetCPURequest())
	td.ResourceRequest.GpuCores = uint64(pod.GetGPURequest())
...
func (pw *PodWatcher) addTaskToJob(pod *Pod, jdUid string, jdName string, tdID int) *firmament.TaskDescriptor {
	task := &firmament.TaskDescriptor{
		...
		ResourceRequest: &firmament.ResourceVector{
			// TODO(ionel): Update types so no cast is required. GPUorCPURequest
			CpuCores:     float32(pod.GetCPURequest()),
			GpuCores:     uint64(pod.GetGPURequest()),
...
func (p *Pod)GPUorCPURequest() int64 {
...
func (p *Pod)GetGPURequest()int64{
	return p.GPURequest
}

func (p *Pod)GetCPURequest()int64{
	return p.CPURequest
}
