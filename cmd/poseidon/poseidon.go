...
//    "github.com/kubernetes-sigs/poseidon/pkg/flowscheduler"
	...
	for {
		...
		for _, delta := range deltas.GetDeltas() {
			...
			case firmament.SchedulingDelta_PLACE:
				...
				glog.Infof("write to channel %v",delta)
				k8sclient.WaitForResReady(podIdentifier.Namespace,podIdentifier.Name,nodeName)
...
func main() {
	...
	glog.Infof("FlowScheduler Policy: Priority :%t, Task Resource Need Num:%t, Resource node availiable resource:%t, Task Running Time:%t.",config.GetEnablePriority(),config.GetEnableTaskResNeed(),config.GetEnableResAvai(),config.GetEnableRunTime())  
	...
	k8sclient.New(config.GetSchedulerName(), config.GetKubeConfig(), kubeMajorVer, kubeMinorVer, config.GetFirmamentAddress(),fc)
}
