...
func (nw *NodeWatcher) parseNode(node *v1.Node, phase NodePhase) *Node {
	...
	gpuCapQuantity := node.Status.Capacity[NVIDIAGPU]
	gpuAllocQuantity := node.Status.Allocatable[NVIDIAGPU]
	...
	return &Node{
		...
		GPUCapacity:      gpuCapQuantity.ScaledValue(0) * NVIDIAGPUMLTI + NVIDIAGPUMLTI/2,
		GPUAllocatable:   gpuAllocQuantity.ScaledValue(0) * NVIDIAGPUMLTI + NVIDIAGPUMLTI/2,
		...
		func() {
			...
			for _, item := range items {
				...
				case NodeDeleted:
					...
					NodeMux.RUnlock()
					if !ok {
						//Node deleted might NOT exists in NodeToRTND
						//glog.Fatalf("Node %s does not exist", node.Hostname)
						glog.Infof("Node %s does not exist", node.Hostname)
						continue
					}
				...
				case NodeFailed:
					...
					NodeMux.RUnlock()
					if !ok {
						// A failed Node might NOT exists in NodeToRTND
						// glog.Fatalf("Node %s does not exist", node.Hostname)
						glog.Infof("Node %s does not exist", node.Hostname)
						continue
					}
				...
				case NodeUpdated:
					...
					rtnd, ok := NodeToRTND[node.Hostname]
					if !ok {
						// An Noded updated NOT exists should be an node Added
						// glog.Fatalf("Node %s does not exist", node.Hostname)
						NodeMux.RUnlock()
						glog.Infof("Node %s (updated) does not exist, treat it as added", node.Hostname)
						NodeMux.Lock()
						rtnd := nw.createResourceTopologyForNode(node)
						NodeToRTND[node.Hostname] = rtnd
						glog.Info(NodeToRTND, " in Nodedded")
...
func (nw *NodeWatcher) createResourceTopologyForNode(node *Node) *firmament.ResourceTopologyNodeDescriptor {
	...
	rtnd := &firmament.ResourceTopologyNodeDescriptor{
		ResourceDesc: &firmament.ResourceDescriptor{
			...
			// CpuCores:     float32(node.GPUorCPUCapacity() - node.GPUorCPUAllocatable()),
			ResourceCapacity: &firmament.ResourceVector{
				RamCap:       uint64(node.MemCapacityKb),
				CpuCores:     float32(node.GetCPUCapacity()),
				GpuCores:     uint64(node.GetGPUCapacity()),
				EphemeralCap: uint64(node.EphemeralCapKb),
			},
			AvailableResources: &firmament.ResourceVector{
				RamCap:       uint64(node.MemAllocatableKb),
				CpuCores:     float32(node.GetCPUAllocatable()),
				GpuCores:     uint64(node.GetGPUAllocatable()),
			...
			ReservedResources: &firmament.ResourceVector{
				...
				CpuCores:     float32(node.GetCPUCapacity() - node.GetCPUAllocatable()),
				GpuCores:     uint64(node.GetGPUCapacity() - node.GetGPUAllocatable()),
...
func (n *Node)GPUorCPUCapacity() int64 {
	if config.GetSchedulingGPU() {
		return n.GPUCapacity
	} else {
		return n.CPUCapacity
	}
}

func (n *Node)GetGPUCapacity() int64 {
	return n.GPUCapacity
}


func (n *Node)GetCPUCapacity() int64 {
    return n.CPUCapacity
}

func (n *Node)GPUorCPUAllocatable() int64 {
	if config.GetSchedulingGPU() {
		return n.GPUAllocatable
	} else {
		return n.CPUAllocatable
	}
}

func (n *Node)GetCPUAllocatable() int64 {
    return n.CPUAllocatable
}

func (n *Node)GetGPUAllocatable() int64 {
    return n.GPUAllocatable
}
